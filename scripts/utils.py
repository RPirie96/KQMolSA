...
def new_coeff(a, b, c, alpha, beta, gamma, delta):
    ...
    k_1 = np.conj((((alpha - a * gamma) * np.conj(beta - a * delta)) + b * gamma * np.conj(delta)))
...
def distance(a_herm, b_herm):  # This function computes the symmetric space distance between two Hermitian matrices A,B
    """
    ...
    @return:
    """
    w, v = la.eig(a_herm)  # Find the eigenvalues w, and a NxN matrix of Id-orthonormal eigenvctors (columns are the vectors)
    for i in range(0, len(a_herm)):
        for j in range(0, len(a_herm)):
            v[i, j] = np.sqrt(1 / w[j]) * v[i, j]  # The matrix v now has as rows an A-ONB

    b_tilde = np.matmul(np.matmul(v.getH(), b_herm), v)  # this is inner product defined by B in the A-ONB
    w_tilde, v_tilde = la.eig(b_tilde)  # now find eigenvalues of B_tilde

    d = 0
    for i in range(0, len(a_herm)):
        d += (np.log(w_tilde[i].real)) ** 2  # distance is sum of squares of log of eigenvalues of B_tilde
...
def conj(x, a_herm):  # This function
    ...
    return x[6] * np.matmul(np.matmul(s_ast, a_herm), s)  # we also allow for multiplication by our scale factor x[6]
...
def get_score(query, test, query_id=None, test_id=None):
    ...
    if query_id is not None:
        if query_id == test_id:
            ...
            x0 = res.x
            return round((1 / (1 + distance(conj(x0, query), test))), 3)  # get score between matrices
    else:
        x0 = np.array([1, 0, 0, 0, 0, 0, 1])  # identity rotation array
